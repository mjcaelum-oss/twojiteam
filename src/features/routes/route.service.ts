...
export async function requestRoute(origin: Spot, destination: Spot, mode: TransportMode): Promise<CalculatedRoute> {
  try {
    ...
    const result = await service.route({
      ...
      ...(mode === 'TRANSIT' ? { transitOptions: { departureTime: new Date(Date.now() + 3600000) } } : {})
    });
