...
export function TravelMap({ spots, selected, current, routes = [], onError }: { spots: Spot[]; selected: Spot[]; current?: Spot; routes?: MapRoute[]; onError?: (message: string) => void }) {
  const element = useRef<HTMLDivElement>(null); const map = useRef<google.maps.Map | null>(null); const directions = useRef<google.maps.DirectionsRenderer[]>([]); const markers = useRef<SpotMarker[]>([]); const [ready, setReady] = useState(false); const [legendExpanded, setLegendExpanded] = useState(false);
  ...
  useEffect(() => { if (!map.current || !ready) return; clearMarkers(markers.current); markers.current = createMarkers(map.current, mapSpots, current?.id, new Set(selected.map((spot) => spot.id))); fitMapToSpots(map.current, mapSpots); }, [ready, mapSpots, current?.id, selected]);
  useEffect(() => {
  ...
  const collapsibleLegend = routes.length >= 5;
  return <div className={styles.panel} aria-label="관광지 지도"><div ref={element} className={styles.map} /><div className={styles.status} hidden={ready}>지도를 불러오려면 Google Maps 키가 필요합니다.<br />mock 데이터와 추천 흐름은 지도 없이도 사용할 수 있어요.</div>{selected.length > 0 && <div className={styles.legend}>선택한 장소 {selected.length}곳</div>}{routes.length > 0 && <div className={`${styles.routeLegend} ${collapsibleLegend && !legendExpanded ? styles.routeLegendCollapsed : ''}`}><div className={styles.routeItems}>{routes.map((segment, index) => <span key={`${segment.origin.id}-${segment.destination.id}`}><i style={{ backgroundColor: routeColors[index % routeColors.length] }} />{index + 1}. {segment.origin.name} → {segment.destination.name}</span>)}</div>{collapsibleLegend && <button type="button" className={styles.legendToggle} aria-expanded={legendExpanded} onClick={() => setLegendExpanded((expanded) => !expanded)}>{legendExpanded ? '범례 접기' : `범례 펼치기 (${routes.length}개)`}</button>}</div>}</div>;
}
