...
export function TravelMap({ spots, selected, current, routes = [], onError }: { spots: Spot[]; selected: Spot[]; current?: Spot; routes?: MapRoute[]; onError?: (message: string) => void }) {
  ...
  useEffect(() => { if (!map.current || !ready) return; clearMarkers(markers.current); markers.current = createMarkers(map.current, mapSpots, current?.id); fitMapToSpots(map.current, mapSpots); }, [ready, mapSpots, current?.id]);
  useEffect(() => {
