...
export function TravelPlanProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<TravelPlanContextValue>(() => ({
    plan, setPlan,
    addSpot: (spot) => setPlan((current) => current && !current.spots.some((item) => item.spot.id === spot.id) ? { ...current, spots: [...current.spots, { spot, visitOrder: current.spots.length + 1, transportMode: current.spots.length ? 'DRIVING' : undefined }], routes: current.spots.length ? [...current.routes, null] : current.routes } : current),
